...
### Comparación con componentes funcionales
...
// El componente funcional abstrae el ciclo de vida
// de un componente en una función directa que devuelve la vista
// y mediante ganchos (hooks) puede retener información
// fuera del ciclo de hidratación y refrescado
export function App() {

    // Los estados son paracialidades ancladas
    // entre valores y ajustadores de valores
    // que se inicializan en un valor primario
    // y evoluvionan mediante el ajustador
    const [user, setUser] = useState(null)

    // Los efectos causan cambios en el estado
    // del componente, cuando se observan las dependencias
    // que son generalmente otros estados
    // cuando no hay dependencias, se asume que el efecto
    // será global sobre el montaje y desmontaje del componente
    // Dentro del efecto se pueden condicionar los cambios
    useEffect(() => {
        console.log("Montado")
    ...
    // Los estados pueden determinar la vista final
    if (user) return (
        <span>home</span>
    )

    // También se pueden concetar con otros componentes
    // para reaccionar a sus resultados
    return (
        <LoginA 
